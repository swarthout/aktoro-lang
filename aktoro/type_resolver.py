...
#
#     def visit_ListType(self, node, arg_type):
#         node.elem_type = self.visit(node.elem_type, arg_type.elem_type)
#         return node
#
#     def visit_PrimitiveType(self, node, arg_type):
#         return node
#
#     def visit_DictType(self, node, arg_type):
#         node.key_type = self.visit(node.key_type, arg_type.key_type)
#         node.val_type = self.visit(node.val_type, arg_type.val_type)
#         return node
#
#     def visit_RecordType(self, node, arg_type):
#         for i, field in enumerate(node.fields):
#             node.fields[i] = self.visit(field.ak_type, arg_type.fields[i].ak_type)
#         return node
#
#     def visit_FuncType(self, node, arg_type):
#         for i, param in enumerate(node.param_types):
#             node.param_types[i] = self.visit(param, arg_type.param_types[i])
#         node.return_type = self.visit(node.return_type, arg_type.return_type)
#         return node
#
#     def visit_TypeParameter(self, node, arg_type):
#         self.type_params[node.param] = arg_type
#         return arg_type
#
#     def visit_EmptyTuple(self, node, arg_type):
#         return node
